...
class ElasticSearchClient(MetaClient, ManualClient):
    ...
    def index_corpusfile_metadata(self, corpus_file_paths:list):
        ...
        for data in corpus_data:
            ...
            embedded_fields = data["ParagraphText"]
            embedded_field_name = embedded_fields.apply(lambda t: self._embedder.embed_single_text(t)[0].tolist())

        requests = []
        for i, data in enumerate(corpus_data):
            request = data
            request["_op_type"] = "index"
            request["_index"] = config.corpus_metaIndex
            request["paragraphTextVector"] = embedded_field_name[i]
